...
public class AmethystTridentItem extends Item implements ProjectileItem {
				...
				if (!(f > 0.0F) || playerEntity.isTouchingWaterOrRain()) {
					if (!isAboutToBreak(stack)) {
						...
						if (!world.isClient) {
							...
							if (f == 0.0F) {
								AmethystTridentEntity amethystTridentEntity = new AmethystTridentEntity(world, playerEntity, stack);
								amethystTridentEntity.setVelocity(playerEntity, playerEntity.getPitch(), playerEntity.getYaw(), 0.0F, 2.5F, 1.0F);
								if (playerEntity.isInCreativeMode()) {
									amethystTridentEntity.pickupType = PersistentProjectileEntity.PickupPermission.CREATIVE_ONLY;
								}

								world.spawnEntity(amethystTridentEntity);
								world.playSoundFromEntity(null, amethystTridentEntity, registryEntry.value(), SoundCategory.PLAYERS, 1.0F, 1.0F);
								if (!playerEntity.isInCreativeMode()) {
	...
	public ProjectileEntity createEntity(World world, Position pos, ItemStack stack, Direction direction) {
		AmethystTridentEntity amethystTridentEntity = new AmethystTridentEntity(world, pos.getX(), pos.getY(), pos.getZ(), stack.copyWithCount(1));
		amethystTridentEntity.pickupType = PersistentProjectileEntity.PickupPermission.ALLOWED;
		return amethystTridentEntity;
	}
}
